...
for i, (trainValIndex, testIndex) in enumerate(outer.split(X, y)):
    ...
    # Plotting for each CV outer loop
    plt.figure(figsize=(10, 6))
    # mean square error
    plt.plot(lambdas, meanErrors, 'k-', label='Mean Error')
    # intervals of variance 
    plt.fill_between(lambdas, meanErrors - stdErrors, meanErrors + stdErrors, color='red', alpha=0.2, label='Error Std. Dev.')
    # mean error of lambdaMin
    plt.scatter(lambdas[lambdaMinIndex], meanErrors[lambdaMinIndex], color='blue', label='λ_min')
    # mean error of lambdaStar
    plt.scatter(lambdas[lambdaStarIndex], meanErrors[lambdaStarIndex], color='green', label='λ*')
    # misc settings
    plt.title(f'Tuning Hyperparameters Using Cross Validation')
    plt.xscale('log')
    plt.xlabel('Lambda')
